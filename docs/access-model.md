...
Prefer **per-user** Run As when operators will run `awslogin` on the host. When sessions still land as shared `ec2-user`, `/etc/profile.d/jump-host-aws-session.sh` creates a per-shell AWS home under `~/.cache/jump-host-aws/<session-id>/`, exports `JUMP_HOST_AWS_HOME`, `AWS_CONFIG_FILE`, and `AWS_SHARED_CREDENTIALS_FILE`, and registers a best-effort `EXIT` trap to remove that tree. `awslogin` runs SSO login under the session home, exports short-lived keys into the session credentials file, and strips `sso_*` from the session config copy only—the durable `~/.aws/config` under `/home/ec2-user` is never modified.
