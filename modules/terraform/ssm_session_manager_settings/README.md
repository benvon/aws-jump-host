...
- `linux_shell_profile` (string|null, default `null`): `inputs.shellProfile.linux` for Standard_Stream sessions (POSIX `sh`, max **512** characters). `null` defaults to `. /etc/profile.d/jump-host-login-env.sh 2>/dev/null || true; . /etc/profile.d/jump-host-path.sh 2>/dev/null || true; cd "$HOME"; if [ -f /etc/jump-host-ssm-bashrc ]; then exec /bin/bash --rcfile /etc/jump-host-ssm-bashrc -i; else exec /bin/bash -i; fi` so managed exports (for example `AWS_PROFILE`) and **`~/bin` on `PATH`** are applied before bash starts for every Run As user including `ec2-user`. When present, `/etc/jump-host-ssm-bashrc` sources AWS session isolation and `~/.bashrc`; when absent (for example before first Ansible configure), ordinary `bash -i` keeps interactive startup. Stderr is discarded when a profile.d snippet is absent. Login-only `bash -l` is **not** used here—it tended to drop Standard_Stream sessions. Set to `""` to keep stock `/bin/sh` if you must troubleshoot Ansible `aws_ssm` issues.
