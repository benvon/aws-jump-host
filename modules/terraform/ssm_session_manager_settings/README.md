...
- `linux_shell_profile` (string|null, default `null`): `inputs.shellProfile.linux` for Standard_Stream sessions (POSIX `sh`, max **512** characters). `null` defaults to `. /etc/profile.d/jump-host-login-env.sh 2>/dev/null || true; . /etc/profile.d/jump-host-path.sh 2>/dev/null || true; cd "$HOME"; exec /bin/bash --rcfile /etc/jump-host-ssm-bashrc -i` so managed exports (for example `AWS_PROFILE`) and **`~/bin` on `PATH`** are applied before bash starts for every Run As user including `ec2-user`, then `/etc/jump-host-ssm-bashrc` sources AWS session isolation and `~/.bashrc`. Stderr is discarded when a snippet is absent (for example before first Ansible bootstrap). Login-only `bash -l` is **not** used here—it tended to drop Standard_Stream sessions. Set to `""` to keep stock `/bin/sh` if you must troubleshoot Ansible `aws_ssm` issues.
